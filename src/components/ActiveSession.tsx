...
export default function ActiveSession({
  ...
  return (
    <div className="breathing-bg min-h-[calc(100vh-4rem)] flex flex-col items-center justify-center p-6 relative w-full transition-all duration-1000 select-none">
      ...
      {soundscape !== 'silence' && (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-white/60 dark:bg-zinc-800/60 backdrop-blur-md px-4 py-1.5 rounded-full border border-primary/10 flex items-center gap-2 shadow-sm text-xs font-medium text-emerald-800 dark:text-emerald-300">
          ...
          >
            {isAudioMuted ? <VolumeX className="h-4.5 w-4.5 text-zinc-400" /> : <Volume2 className="h-4.5 w-4.5 text-emerald-600" />}
          </button>
        </div>
      )}
      ...
      <div className="flex flex-col items-center text-center space-y-12 max-w-[1140px] w-full mt-4">
        ...
        <div className="relative w-64 h-64 flex items-end justify-center">
          {/* Pot illustration */}
          <div className="absolute bottom-0 w-32 h-20 bg-secondary-fixed dark:bg-zinc-800 rounded-b-2xl rounded-t-sm border-b-4 border-secondary/20 flex items-center justify-center overflow-hidden shadow-sm z-20">
          ...
          >
            {/* Stem structure */}
            <div className="w-2.5 h-24 bg-primary-container dark:bg-emerald-700/80 rounded-full mx-auto relative shadow-inner">
              
              {/* Leaf 1 (appears always) */}
              <div className="absolute -left-6 top-8 w-8 h-4 bg-primary dark:bg-emerald-800 rounded-full rotate-[-30deg] origin-right shadow-sm"></div>
              ...
              {progress >= 0.25 && (
                <motion.div 
                  ...
                  className="absolute -right-6 top-4 w-8 h-4 bg-primary-container dark:bg-emerald-600 rounded-full origin-left shadow-sm"
                ></motion.div>
              )}
              ...
              {progress >= 0.50 && (
                <motion.div 
                  ...
                  className="absolute -left-5 top-14 w-7 h-3.5 bg-primary/90 dark:bg-emerald-700 rounded-full origin-right shadow-sm"
                ></motion.div>
              )}
              ...
              {progress >= 0.85 ? (
                <motion.div 
                  initial={{ scale: 0 }}
                  animate={{ scale: 1.15 }}
                  className="absolute -top-4 -left-2 w-6 h-6 bg-rose-400 dark:bg-rose-500 rounded-full shadow-md flex items-center justify-center"
                >
                  <div className="w-2.5 h-2.5 bg-amber-300 rounded-full"></div>
                </motion.div>
              ) : (
                <div className="absolute -top-2 left-1/2 -translate-x-1/2 w-4 h-6 bg-primary dark:bg-emerald-800 rounded-full shadow-sm"></div>
              )}
            </div>
          </motion.div>
        ...
        <div className="space-y-2">
          <p className="font-sans text-xs font-semibold tracking-[0.2em] text-secondary dark:text-zinc-400 uppercase">
            {sessionLabel}
          </p>
          <div className="font-sans text-[76px] md:text-[110px] text-on-surface dark:text-zinc-100 font-extralight tracking-tighter leading-none tabular-nums select-none">
        ...
        <div className="h-16 flex items-center justify-center max-w-lg mx-auto">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <p className="font-sans text-base md:text-lg text-secondary dark:text-zinc-300 italic px-4 text-center leading-relaxed">
                "{currentQuote.text}"
              </p>
            </motion.div>
          </AnimatePresence>
      ...
      <div className="fixed bottom-32 md:bottom-24 left-0 w-full flex justify-center px-margin-mobile z-20">
        <button
          ...
          className="group flex items-center gap-2.5 px-10 py-3.5 bg-white/40 dark:bg-zinc-900/40 border border-outline-variant/60 dark:border-zinc-800 hover:bg-secondary-fixed/40 dark:hover:bg-zinc-800 text-secondary dark:text-zinc-300 font-sans text-xs font-semibold uppercase tracking-wider rounded-full shadow-sm hover:shadow active:scale-95 cursor-pointer backdrop-blur-sm transition-all"
        >
